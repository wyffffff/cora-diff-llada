...
OUT_DIR="output/three_method_compare/${TASK}_g${GEN_LENGTH}_s${STEPS}_${SAMPLE_TAG}"
mkdir -p "$OUT_DIR"
...
CORA_BEST_ARGS="${CORA_BASE_ARGS},cora_residual_accept=false,cora_residual_threshold=0.0,cora_drift_topk=1,cora_residual_beta=0.0,cora_residual_gamma=0.0,cora_persistence_temperature=${CORA_PLUS_PERSISTENCE_TEMPERATURE}"
CORA_PLUS_ARGS="${CORA_BASE_ARGS},cora_residual_accept=true,cora_residual_threshold=${CORA_PLUS_RESIDUAL_THRESHOLD},cora_drift_topk=${CORA_PLUS_DRIFT_TOPK},cora_residual_beta=${CORA_PLUS_RESIDUAL_BETA},cora_residual_gamma=${CORA_PLUS_RESIDUAL_GAMMA},cora_persistence_temperature=${CORA_PLUS_PERSISTENCE_TEMPERATURE}"
...
echo "CORA_BEST=residual_accept=false confidence=${CORA_ACCEPT_CONFIDENCE_THRESHOLD} stability=${CORA_ACCEPT_STABILITY_STEPS} eot=${CORA_EOT}"
echo "CORA_PLUS=residual_accept=true threshold=${CORA_PLUS_RESIDUAL_THRESHOLD} topk=${CORA_PLUS_DRIFT_TOPK} beta=${CORA_PLUS_RESIDUAL_BETA} gamma=${CORA_PLUS_RESIDUAL_GAMMA}"

run_eval "original" "$ORIGINAL_ARGS"
run_eval "learn2pd" "$LEARN2PD_ARGS"
run_eval "cora_best" "$CORA_BEST_ARGS"
run_eval "cora_plus" "$CORA_PLUS_ARGS"
