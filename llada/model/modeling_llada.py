...
from .cora_diff import CoraDiffState, routed_llama_mlp
...
class LLaDABlock(nn.Module):
    ...
    def attention(
        ...
        cora_state: Optional[CoraDiffState] = None,
        ...
        if cora_state is not None:
            cora_state.set_attention_features(self.layer_id, q, k)
    ...
    def forward(
        ...
        replace_position: Optional[torch.Tensor] = None,
        cora_state: Optional[CoraDiffState] = None,
        cora_unresolved_mask: Optional[torch.Tensor] = None,
    ) -> Tuple[torch.Tensor, Optional[Tuple[torch.Tensor, torch.Tensor]]]:
        raise NotImplementedError
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, cora_state=cora_state
            )
        else:
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, cora_state=cora_state)
...
class LLaDALlamaBlock(LLaDABlock):
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            ...
            x = self.ff_norm(x)
        if cora_state is not None:
            levels = cora_state.refinement_levels(self.layer_id, x, cora_unresolved_mask)
            x = routed_llama_mlp(
                x,
                levels,
                self.ff_proj,
    ...
    def forward(
        ...
        if self._activation_checkpoint_fn is not None:
            att, cache = self._activation_checkpoint_fn(  # type: ignore
                self.attention, q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position, cora_state=cora_state
            )
        else:
            att, cache = self.attention(q, k, v, attention_bias, layer_past=layer_past, use_cache=use_cache, replace_position=replace_position, cora_state=cora_state)
        ...
        for block_idx, block in enumerate(self):
            ...
            if (
                ...
                x, cache = self._activation_checkpoint_fn(  # type: ignore
                    ...
                    use_cache=use_cache,
                    cora_state=cora_state,
                    cora_unresolved_mask=cora_unresolved_mask,
                )
            else:
                # shape: (batch_size, seq_len, d_model)
                x, cache = block(
